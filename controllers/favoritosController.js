...
module.exports ={
    async listarFavoritos(request, response) {
        try{
            ...
            const sql = 'SELECT f.usu_id, f.jogo_id, f.Status FROM favoritos f INNER JOIN usuario u ON f.usu_id = u.usu_id  INNER join jogos j ON f.jogo_id = j.jogo_id  WHERE f usu_id LIKE ? AND f.jogo_id LIKE ?  AND f.status LIKE ? LIMIT 0, 10 ;'


            const values = [ jogo_Id, usu_Id, parseInt(limit)];
